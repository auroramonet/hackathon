...
async function getPopulationData(south, west, north, east) {
  try {
    ...
    console.log(
      `🔍 Getting census data for center point: ${centerLat.toFixed(
        6
      )}, ${centerLon.toFixed(6)}`
    );
    ...
    const bboxArea =
      earthRadius * earthRadius * latDiff * lonDiff * Math.cos(avgLat);
    ...
    console.log(
      `🎯 Estimated Population in Area: ${estimatedPopulation.toLocaleString()}`
    );
  ...
  try {
    ...
    return {
      ...
      summary: {
        ...
        total_critical: Object.values(critical_facilities).reduce(
          (a, b) => a + b,
          0
        ),
      },
...
app.post("/api/analyze", async (req, res) => {
  try {
    ...
    if (!bbox) {
      return res
        .status(400)
        .json({ error: "Could not calculate bounding box from coordinates" });
    }

    console.log(
      `📍 Bounding box: ${bbox.south.toFixed(4)}, ${bbox.west.toFixed(
        4
      )}, ${bbox.north.toFixed(4)}, ${bbox.east.toFixed(4)}`
    );
    ...
    console.log(
      `👥 Population found: ${populationData.total_population.toLocaleString()}`
    );
    console.log(
      `🏠 Buildings found: ${buildingData.total_buildings.toLocaleString()}`
    );
    console.log(
      `🏥 Critical facilities: ${buildingData.summary.total_critical}`
    );
    ...
    const realWorldData = {
      ...
      population_density:
        areaSize > 0
          ? Math.round(populationData.total_population / areaSize)
          : 0,
    };
...
Major Places: ${realWorldData.population.places
      .slice(0, 3)
      .map((p) => `${p.name} (${p.population.toLocaleString()})`)
      .join(", ")}
...
1. [Prioritize based on ${
      realWorldData.buildings.summary.total_critical
    } critical facilities]
2. [Focus on ${
      realWorldData.buildings.summary.critical_facilities.hospitals
    } hospitals, ${
